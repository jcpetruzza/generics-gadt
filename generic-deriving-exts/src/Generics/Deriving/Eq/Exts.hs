...
instance GEq' (SubstSk free t) => GEq' (GEx free '[] ftvars btvars t) where
  geq' (QF l) (QF r) = geq' l r

instance
  ( GEq' (GEx (x :> v ': free) bound (Ty v ': ftvars) btvars t)
  ) => GEq' (GEx free (V x k K ': bound) ftvars (Ty v ': btvars) t) where
