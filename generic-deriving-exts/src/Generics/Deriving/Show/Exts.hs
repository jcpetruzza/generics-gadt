...
instance GShow' (SubstSk free t) => GShow' (GEx free '[] ftvars btvars refined a t) where
  gshowsPrec' ty n (QF t) = gshowsPrec' ty n t
  isNullary (QF t) = isNullary t


instance
  (forall (v :: k) ftvars' btvars' . GShow' (GEx (x :> v ': free) bound ftvars' btvars' refined a t))
    => GShow' (GEx free (V x k K ': bound) ftvars btvars refined a t) where
