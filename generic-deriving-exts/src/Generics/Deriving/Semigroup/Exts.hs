...
instance GSemigroup' (SubstSk free t) => GSemigroup' (GEx free '[] ftvars btvars t) where
  gsappend' (QF l) (QF r) = QF (gsappend' l r)


instance
  ( GSemigroup' (GEx (x :> v ': free) bound (Ty v ': ftvars) btvars t)
  ) => GSemigroup' (GEx free (V x k K ': bound) ftvars (Ty v ': btvars) t) where
